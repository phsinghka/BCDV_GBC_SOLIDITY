...
contract Escrow{
    ...
    enum TransactionState{AWAITING_PAYMENT, AWAITING_DELIVERY, COMPLETE}
    ...
    function deposit() onlyBuyer() external payable {
        require(currentState == TransactionState.AWAITING_PAYMENT, "Payment has alreay been deposited");
        currentState = TransactionState.AWAITING_DELIVERY;

    } 

    function confirmDelivery() onlyBuyer() external{
        require(currentState == TransactionState.AWAITING_DELIVERY, "As a buyer You have to pay first to Confirm the Delivery");
        transferFunds();
