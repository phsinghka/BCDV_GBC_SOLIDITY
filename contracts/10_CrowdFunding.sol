...
contract FundMe{
    ...
    enum State{Fundraising, Expired, Succesfull}
    ...
    modifier atEndofCycle(){
        require((state==State.Expired || state == State.Succesfull)&& 4<block.timestamp);
        _;
    }



    constructor(uint _amount,uint _days, address payable _recipient){
        owner = msg.sender;
    ...
    function donate(uint _amount) public payable isState(State.Fundraising) returns (uint id){
